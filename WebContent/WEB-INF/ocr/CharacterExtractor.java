...
public class CharacterExtractor {
	...
	private static InputImage inputImage = null;
	...
	public static void processChar(PixelImage pixelImage, File input, int x1, int y1, int x2, int y2, int rowY1,int rowY2) {
		{
			try {
				//inputImage = input;
				...
				BufferedImage characterImage1 = ImageIO.read(inputImage);
	...
	private static void processRow(PixelImage pixelImage, int[] pixels, int w, int h, int x1, int y1, int x2, int y2) {
		...
		for (int i = 0; (i + 1) < al.size(); i += 2) {
			...
			if (cy1 >= cy2) {
				...
				processChar(pixelImage, inputImage, cx1, cy1, cx2, cy2, y1, y2);
