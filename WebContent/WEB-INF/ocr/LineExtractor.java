...
public class LineExtractor {
	...
	private static DocumentScanner documentScanner = new DocumentScanner();
	private static File outputDir = null;
	private static File inputImage = null;

	public static void slice(File inputImage1, File outputDir1) {
		try {
	 ...
	 * row of text.
	 */
	protected static float minSpaceWidthAsFractionOfRowHeight = 0.1f;
	/**
		...
		for (int i = 0; (i + 1) < al.size(); i += 2) {
			if (i >= 2) {
				...
				while ((cx2 - cx1) >= minSpaceWidth) {
					...
					CharacterExtractor.processSpace(pixelImage, cx1, y1, sx2, y2);
					cx1 += minSpaceWidth;
				}
			}
			...
			if (cy1 >= cy2) {
				CharacterExtractor.processChar(pixelImage, inputImage, cx1, cy1, cx2, cy2, y1, y2);
			} else {
				CharacterExtractor.processChar(pixelImage, inputImage, cx1, cy1, cx2, cy2, y1, y2);
			}
		}
